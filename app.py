...
:root{{ 
  ...
  --surface:{'#070707' if IS_DARK else '#FFFCF5'};
  --panel:{'#050505' if IS_DARK else '#F4ECDF'};
  --card:{'#0A0A0A' if IS_DARK else '#FFFCF5'};
  --card-soft:{'#141414' if IS_DARK else '#EFE6D7'};
  --input-bg:{'#050505' if IS_DARK else '#FFFCF5'};
  --border:{'rgba(255,255,255,.16)' if IS_DARK else 'rgba(95,83,65,.12)'};
  --border2:{'rgba(255,255,255,.28)' if IS_DARK else 'rgba(95,83,65,.20)'};
  --input-border:{'rgba(255,255,255,.20)' if IS_DARK else 'rgba(95,83,65,.16)'};
...
.missing-photo-context{{ filter:{'grayscale(.18) saturate(.55) contrast(.82) brightness(.64)' if IS_DARK else 'saturate(.82) contrast(.96)'};opacity:{'.88' if IS_DARK else '1'};}} 
...
:root{{ 
  ...
  --card-soft:{'#141414' if IS_DARK else '#EFE6D7'};
  --input-bg:{'#050505' if IS_DARK else '#FFFCF5'};
  --border:{'#2A2A2A' if IS_DARK else '#D5C9B8'};
  --border2:{'#414141' if IS_DARK else '#BFAF99'};
  --input-border:{'#343434' if IS_DARK else '#D5C9B8'};
  --focus-ring:{'rgba(110,173,122,.20)' if IS_DARK else 'rgba(78,138,96,.16)'};
  --text:{'#F2F1EA' if IS_DARK else '#1F211D'};
  --text2:{'#CDD4C8' if IS_DARK else '#4C5048'};
  --text3:{'#8F9B8D' if IS_DARK else '#6D7168'};
...
[data-testid="stSidebar"] .stButton>button:hover{{ 
  background:{'#141414' if IS_DARK else '#EFE6D7'}!important;
  color:var(--text)!important;
}} 
[data-testid="stSidebar"] .stButton button[kind="primary"],
[data-testid="stSidebar"] .stButton button[kind="primaryFormSubmit"]{{ 
  background:{'#0A0A0A' if IS_DARK else '#FFFCF5'}!important;
  border-color:{'#343434' if IS_DARK else '#D5C9B8'}!important;
  color:var(--text)!important;
}} 
[data-testid="stSidebar"] .stButton button[kind="secondary"]{{ 
  color:var(--text3)!important;
...
.sidebar-nav-photo{{ 
  ...
  background:{'linear-gradient(145deg,#171717 0%,#070707 100%)' if IS_DARK else '#EFE6D7'};
  padding:0;
...
.sidebar-nav-photo img{{ 
  ...
  object-fit:contain;
  display:block;
  filter:{'invert(1) brightness(.76)' if IS_DARK else 'none'};
  opacity:{'.72' if IS_DARK else '.7'};
}} 
.sidebar-nav-photo.active{{ 
  border-color:var(--red);
  background:{'linear-gradient(145deg,#1F1F1F 0%,#0B0B0B 100%)' if IS_DARK else '#FFFCF5'};
  box-shadow:0 0 0 2px var(--red-dim),0 8px 22px rgba(0,0,0,.24);
}} 
.sidebar-nav-photo.active img{{ 
  filter:{'invert(1) brightness(.9)' if IS_DARK else 'none'};
  opacity:{'.88' if IS_DARK else '.82'};
}} 
...
[data-testid="stSidebar"] .theme-toggle-wrap + div [data-testid="stHorizontalBlock"]{{ 
  ...
  background:{'#111111' if IS_DARK else '#EFE6D7'}!important;
  border:1px solid {'#343434' if IS_DARK else '#D5C9B8'}!important;
}} 
.page-header{{ 
...
.stat-card,.case-card,.dash-case-card,.dash-vol-card,.vol-card,.form-card,.log-item{{ 
  box-shadow:{'inset 0 1px 0 rgba(255,255,255,.045), 0 12px 30px rgba(0,0,0,.34)' if IS_DARK else 'var(--shadow-sm)'}!important;
}} 
...
.dash-case-card .missing-photo-context,
.case-card .missing-photo-context{{ 
  filter:{'grayscale(.24) saturate(.48) contrast(.82) brightness(.58)' if IS_DARK else 'saturate(.82) contrast(.96)'}!important;
  opacity:{'.86' if IS_DARK else '1'}!important;
  box-shadow:{'inset 0 0 0 1px rgba(255,255,255,.05), 0 12px 28px rgba(0,0,0,.34)' if IS_DARK else 'var(--shadow-sm)'}!important;
}} 
.missing-photo-fallback{{ 
  ...
  justify-content:center;
  gap:6px;
  background:{'linear-gradient(145deg,#151515 0%,#050505 100%)' if IS_DARK else '#EFE6D7'}!important;
}} 
.missing-photo-fallback img{{ 
  width:30px;
  height:30px;
  object-fit:contain;
  filter:{'invert(1) brightness(.88)' if IS_DARK else 'none'};
  opacity:{'.74' if IS_DARK else '.7'};
}} 
.missing-photo-fallback span{{ 
  max-width:72px;
  color:var(--text3);
  font-family:var(--font-mono);
  font-size:.48rem;
...
.stButton>button:hover{{ 
  background:{'#141414' if IS_DARK else '#EFE6D7'}!important;
  border-color:{'#414141' if IS_DARK else '#BFAF99'}!important;
...
.stTextInput input,.stTextArea textarea,.stNumberInput input,
div[data-baseweb="select"]>div{{ 
  background:{'#050505' if IS_DARK else '#FFFCF5'}!important;
  border-color:{'#343434' if IS_DARK else '#D5C9B8'}!important;
  border-radius:13px!important;
  color:var(--text)!important;
}} 
        ...
        for pid,label in pages:
            ...
            with img_col:
                ...
                st.markdown(
                    ...
                    f'<div class="sidebar-nav-photo{active_class}"><img src="{nav_photo_url(pid)}" alt="{label} icon" loading="lazy"></div>',
...
def page_tracking():
    ...
    map_card = "#0A0A0A" if IS_DARK else "#FFFCF5"

    map_border = "#343434" if IS_DARK else "#D5C9B8"
